...
1. **CycleGAN Backbone**
   - Cycle Consistency Loss를 통해 A(청년) → B(중장년) → A(청년) 복원 과정을 학습하며, 변환 과정에서 언어 정보가 유실되지 않도록 합니다.

2. **Speaker Consistency (화자 일관성 유지)**
   - Pre-trained Speaker Encoder를 사용해 변환 전후 화자 임베딩 유사도(Cosine Similarity)를 계산합니다.
...
3. **Age Knowledge Distillation (나이 특징 주입)**
   ...
   - Generator가 생성한 목소리가 목표 나이대(Target Age Group)의 특징을 갖도록 강제합니다.
...
- **Data Source:** YouTube Music / Live Performance Videos (MR 제거 및 보컬 추출 완료)
- **Composition:**
